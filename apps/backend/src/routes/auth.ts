...
router.get("/verify", async (req, res) => {
  ...
  const valid = emailTokens.get(email as string) === token;
  if (!valid) return res.status(400).json({ error: "Invalid token" });
  await db.update(users).set({ email_verified: 1 }).where(eq(users.email, email as string));
  emailTokens.delete(email as string);
